...
clone(void(*fcn)(void *, void *), void *arg1, void *arg2, void *stack)
{
  ...
  //check if there is space in process to accomodate one page size
  //of stack. 
  if ((curproc->sz-(uint)stack)<PGSIZE)
	  return -1;

  //point the page directory to the parent page directory
 ...
 //   np->thread_stack = ((void*)stack)[0];
  ...
  user_stack_contents[2] = (uint)arg2;
  stack_ptr = stack_ptr - 12;
  
  //copy contents of the stack into thread's stack. 
  if (copyout(np->pgdir,stack_ptr,user_stack_contents,12)<0)
	 return -1; 
...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc)
        continue;
      // check if the process is a thread
